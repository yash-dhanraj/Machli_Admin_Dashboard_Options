...
export default function ManualNotificationsLayout({ vm }: { vm: ManualNotificationVm }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeader title="Manual Notifications" subtitle="Compose and send a notification to Machli app users" breadcrumb="Manual Notifications" />
      ...
      <div className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white p-2.5 shadow-sm">
        {MANUAL_NOTIFICATION_STEPS.map((label, idx) => (
          <div key={label} className="flex min-w-0 flex-1 items-center gap-1.5">
            ...
            >
              {idx < vm.step ? <Check size={12} /> : idx + 1}
            </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_300px]">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          {vm.step === 2 && (
            <div className="space-y-3">
              <h2 className="text-sm font-semibold text-navy-900">Step 3 · Preview & Recipients</h2>
        ...
        <div className="h-fit rounded-xl border border-slate-200 bg-surface-alt p-4">
          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Live Summary</p>
          <div className="space-y-2 text-xs">
            <div>
              <p className="text-slate-400">Type</p>
              <p className="font-medium text-navy-800">{vm.notifType || 'Not selected'}</p>
            </div>
            <div>
              <p className="text-slate-400">Title</p>
              <p className="truncate font-medium text-navy-800">{vm.title || '—'}</p>
            </div>
            <div>
              <p className="mb-1 text-slate-400">Recipients</p>
      ...
      <div className="flex items-center justify-between">
        <button
          onClick={vm.goBack}
          disabled={vm.step === 0}
          className="flex items-center gap-1.5 rounded-lg border border-slate-300 px-3.5 py-1.5 text-sm font-medium text-navy-800 hover:bg-surface-alt disabled:cursor-not-allowed disabled:opacity-40"
        >
        ...
        {vm.step < MANUAL_NOTIFICATION_STEPS.length - 1 ? (
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg bg-navy-900 px-3.5 py-1.5 text-sm font-medium text-white hover:bg-navy-800 disabled:cursor-not-allowed disabled:opacity-40"
          >
          ...
          <button
            onClick={() => vm.send()}
            className="flex items-center gap-1.5 rounded-lg bg-teal-600 px-3.5 py-1.5 text-sm font-medium text-white hover:bg-teal-500"
          >
